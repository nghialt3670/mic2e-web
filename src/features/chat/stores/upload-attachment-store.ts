...
export const useUploadAttachmentStore = create<UploadAttachmentStore>((set, get) => ({
  files: [],
  ...
  setAttachments: (attachments: UploadAttachment[]) =>
    set({
      filenameToAttachmentMap: attachments.reduce(
        (acc, attachment) => ({ ...acc, [attachment.originalName]: attachment }),
        {},
      ),
    }),
  ...
  getAttachment: (filename: string) => get().filenameToAttachmentMap[filename],
  ...
  updateAttachmentUploadInfo: (filename: string, uploadInfo: UploadInfo) =>
    set((state) => ({
      filenameToAttachmentMap: {
        ...state.filenameToAttachmentMap,
        [filename]: { ...state.filenameToAttachmentMap[filename], uploadInfo },
      },
    })),
