...
import { v4 } from "uuid";
import { LucideIcon, MousePointerClick, SquareDashedMousePointer, SquareMousePointer } from "lucide-react";
...
export const useInteractionStore = create<InteractionStore>((set, get) => ({
  ...
  getCurrentReference: () => get().referenceStack[get().referenceStack.length - 1],
  updateCurrentType: (type: InteractionType) => set((state) => ({ referenceStack: state.referenceStack.map((reference) => reference.id === state.referenceStack[state.referenceStack.length - 1].id ? { ...reference, type } : reference) })),
  addReference: () => set((state) => ({ referenceStack: [...state.referenceStack, createReference()] })),
  removeReference: (id) => set((state) => ({ referenceStack: state.referenceStack.filter((reference) => reference.id !== id) })),
