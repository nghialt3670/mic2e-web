...
export const UploadAttachmentList = () => {
  const { filenameToFileMap, filenameToPathMap, removeAttachment } = useAttachmentStore();
  ...
  return (
    <div className="flex flex-col gap-2">
      {Object.values(filenameToFileMap).map((file) => (
        <UploadAttachmentItem key={file.name} file={file} onRemove={handleRemoveAttachment} />
      ))}
    </div>
  );
};